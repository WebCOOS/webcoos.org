import { useState, useEffect, useMemo, useContext, type ReactElement } from 'react';
...
import { Loader, utils, ViewWithLoader } from '@axdspub/axiom-ui-utilities';
...
function Cameras(
    ...
    const [curCameras, setCurCameras] = useState<IWebCOOSParsedAsset[]>([]);
    ...
    useEffect(() => {
        const getCurrentCams = async () => {
            ...
            try {
                result = await fetchAPIAssets({ apiUrl: apiUrl, apiVersion: apiVersion, token: token, source: source });
            } catch (e) {
            ...
            const parsedCams = result.map((item) => {
            ...
            <Page title='Cameras'>
                <Section>
                    <SectionHeader>
                        ...
                        {isLoading && <Loader className={'inline-block ml-1 text-primary'} />}
                    </SectionHeader>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className='py-2 px-1 lg:px-2 text-left text-xs font-mono align-middle'>
                                            {isLoading ? (
                                                <Loader className={'inline-block ml-1 text-primary'} />
                                            ) : (
                                                dateRanges &&
                                                dateRanges[c.slug] &&
